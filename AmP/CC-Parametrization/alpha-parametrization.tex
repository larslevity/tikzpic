...
%%%% Draw Angle Phi
\draw[help line] (K)--++(3.6,0);
\draw[angle arrow] (K)++(-\phival:3.5)arc(-\phival:0:3.5);
\path (K)++(-\phival*.5:4.1)node{$\phi$};
...
\draw[->] (1)--++(1,0,0) node[right]{$z_1$};
\draw[->] (1)--++(0,1,0) node[left]{$x_1$};
\draw[->] (1)--++(0,0,1) node[above]{$y_1$};
...
\pgfmathsetmacro{\nx}{(cos(\phival)^2*(cos(\theta)-1)+1)*\scale}
...
\pgfmathsetmacro{\ey}{(sin(\phival)^2*(cos(\theta)-1)+1)*\scale}
