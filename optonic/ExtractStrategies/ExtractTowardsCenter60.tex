...
\draw[->, red] (x)--++(1,0,0);
\draw[->, green] (x)--++(0,1,0);
\draw[help lines] (x)--(0,0,\zgrasp+5);
\draw[->, blue] (x)--++(0,0,1);
...
\draw[help lines] (grasp)--(0,0,\zgrasp+4.5)coordinate[pos=.5](retract) coordinate[pos=.28](start);
\draw[help lines] (grasp)--(0,0,\zgrasp)coordinate[pos=.4](end);
\draw[very thick,-latex, red](grasp)--(retract);

%% draw angle
\draw[<-] (start)to[out=-40, in=90](end);
\draw (end)++(0,-.1,.2) node [above left]{$60^\circ$};
