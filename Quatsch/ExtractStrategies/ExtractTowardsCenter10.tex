...
\draw[->, red] (x)--++(1,0,0);
\draw[->, green] (x)--++(0,1,0);
\draw[help lines] (x)--(0,0,\zgrasp+1);
\draw[->, blue] (x)--++(0,0,1);
...
\draw[help lines] (grasp)--(0,0,\zgrasp+.5)coordinate[pos=.7](retract) coordinate[pos=.4](start);
\draw[help lines] (grasp)--(0,0,\zgrasp)coordinate[pos=.4](end);
\draw[very thick,-latex, red](grasp)--(retract);

%% draw angle
\draw[<-] (start)to[bend left=3mm](end);
\draw (start)++(0,-.1,-.15) --++(0,.5,-.3) node [right]{$10^\circ$};
...
\draw[|<->|, help lines] ($(grasp)+(0,0,.1)$)--($(retract)+(0,0,.1)$) node[midway, above, sloped] {extract distance};
