...
\def\lastbit{0}
\foreach[count=\i] \bit in {1,1,1,0,0,0,0,0,0}{
	% Notes
	\pgfmathtruncatemacro{\j}{8-\i}
	\def\text{A\j}
	\def\fillcol{master}
	\ifnum\i=8 \def\text{W} \fi
...
%% Write on Slave
\pgfmathsetmacro{\startx}{\lastx}
\foreach[count=\i] \bit in {0,0,0,1,0,0,0,0,0}{
	% Notes
