...
% Basics
\draw[base] (client)circle(\r);
\draw[base] (server)++(180:\r)--($(server)+(-.2,0)$)|-($(server)+(90+45:\r)$)coordinate(X)arc(90+45:-180:\r);

% Nodes
\path (client)++(-\rh+.1,-\rh-.2)node[socket]{bound \\ socket A};
\path (server)++(\rh-.15,-\rh-.2)node[socket]{bound \\ socket C};
\path (server)++(\rh-.15,.1)node[socket]{bound \\ socket B};

\draw[queue] (client)++(.2,-.2)rectangle($(server)+(-.2,-.7)$)node[midway]{Connection};
...
\begin{itemize}
\item \textsl{Server} hat Anfrage akzeptiert.
\item \textsl{Server} hat die Verbindung mit \\ einem neuen Socket etabliert.
\item Ursprüngliches Socket wartet \\ auf weitere Anfragen.
\end{itemize}}
};
