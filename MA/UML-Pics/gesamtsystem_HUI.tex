...
\umlnote[x=-8,y=-4,width=4cm]{Communication Interface-interface}{Basierend auf BSD-Socket-Interface (\url{https://docs.python.org/2/library/socket.html})}
...
\begin{umlcomponent}[x=-11,y=-8, fill=gray!5, name=HUI]{Hardware User Interface}
\umlbasiccomponent[level2]{PushBtns}
\umlbasiccomponent[y=-2, level2]{Potentiometer}
%\umlassemblyconnector{GUI-south-port}{Communication-north-port}
%\umlrequiredinterface[padding=1cm, width=0.1]{GUI}
\end{umlcomponent}


\umlactor[x=-6, y=-11, scale=2]{User}

\umlVHassemblyconnector[interface=Alt User Interface, anchor2=-10]{User}{HUI}
\draw (CBoard-west-port) ++(-.2,0)--++(-.2,0)--++(0,-5.5)coordinate(XX);
\umlVHassemblyconnector[interface=Alt Communication Interface, anchor1=36]{HUI}{XX}
